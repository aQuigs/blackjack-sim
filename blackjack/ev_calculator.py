...
class EVCalculator:
    ...
    def calculate_evs(self, graph: StateTransitionGraph) -> dict[State, StateEV]:
        ...
        for state in reversed(sorted_states):
            ...
            if state not in transitions:
                raise ValueError(f"State {state} not found in transitions")
    ...
    def _initialize_terminal_states(
        ...
        for state in transitions:
            if isinstance(state, TerminalState):
                payout = self.rules.get_outcome_payout(state.outcome)
                state_evs[state] = StateEV(Action.GAME_END, {Action.GAME_END: payout})
    ...
    def _get_state_ev(self, state: State, state_evs: dict[State, StateEV]) -> float:
        if state not in state_evs:
            raise ValueError(f"State {state} not found in state_evs")
