...
def main(printable: bool = True):
    ...
    if printable:
        for i, player in enumerate(game.players):
            print(f"Player {i+1} initial hand: {[str(card) for card in player.hand.cards]}")
        print(f"Dealer initial hand: {[str(card) for card in game.dealer.hand.cards]}")
    ...
    for i, player in enumerate(game.players):
        while True:
            if rules.is_bust(player.hand):
                if printable:
                    print(f"Player {i+1} busts with {[str(card) for card in player.hand.cards]}")
                break
            ...
            if action == Action.HIT:
                ...
                if printable:
                    print(f"Player {i+1} stands with {[str(card) for card in player.hand.cards]}")
                break
            else:
    ...
    # Dealer turn
    if printable:
        print(f"Dealer reveals hand: {[str(card) for card in game.dealer.hand.cards]}")
    ...
    if printable:
        print(f"Dealer final hand: {[str(card) for card in game.dealer.hand.cards]}")
